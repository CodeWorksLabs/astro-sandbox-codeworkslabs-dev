...
test('actual source and destination UI use the isolated handoff boundary', () => {
	const source = readFileSync(new URL('../src/pages/analytics.astro', import.meta.url), 'utf8');
	...
	assert.match(source, /const journeyStorageKey = 'cwl-astro-journey-result'/);
	...
	const destination = readFileSync(new URL('../src/pages/analytics/next.astro', import.meta.url), 'utf8');
